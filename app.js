...
function count(s){
  const now=Date.now();
  let a=[];
  try{a=JSON.parse(localStorage.getItem(key(s))||"[]")}catch(e){}
  a=a.filter(t=>now-t<86400000);
  a.push(now);
...
function breakoutLevel(c){
  if(!c||c.length<2)return null;
  const current=c.at(-1).close;
  const prior=c.slice(0,-1).map(x=>x.close).filter(Number.isFinite);
  const below=prior.filter(p=>p<current);
  return below.length?Math.max(...below):Math.max(...prior)
}
function drawBreakout(c){
  if(!candleSeries)return;
  const level=breakoutLevel(c);
