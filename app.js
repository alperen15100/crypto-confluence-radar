...
function key(s){return"fresh1d:"+s}function count(s){let n=Date.now(),a=[];try{a=JSON.parse(localStorage.getItem(key(s))||"[]")}catch(e){}a=a.filter(t=>n-t<86400000);let last=a.at(-1)||0;if(!last||n-last>=3600000)a.push(n);try{localStorage.setItem(key(s),JSON.stringify(a))}catch(e){}return a.length}
...
$("refreshBtn").onclick=scan;$("search").oninput=render;$("zonesBtn").onclick=()=>{zonesOn=!zonesOn;$("zonesBtn").textContent=zonesOn?"AUTO CONFLUENCE ON":"AUTO CONFLUENCE";if(selected)openChart(selected)};
